...
export default function MeetingBookingsPage() {
  ...
  const [filteredBookings, setFilteredBookings] = useState<MeetingBooking[]>([])
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        ...
        <span className="ml-2 text-muted-foreground">Loading meeting bookings...</span>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-3xl font-bold">Meeting Bookings</h1>
          <p className="text-muted-foreground">
            Manage and organize scheduled meetings with clients.
          </p>
        </div>
      </div>
          ...
          {filteredBookings.length === 0 ? (
            <Card>
              <CardContent className="flex flex-col items-center justify-center h-60">
                ...
                <p className="text-muted-foreground">No meeting bookings found</p>
                {filter !== "all" && (
                  ...
                  <TableBody>
                    {filteredBookings.map((booking) => (
                      <TableRow key={booking.id}>
                        ...
                        <TableCell className="max-w-[180px] truncate">
                          {booking.courseId ? (
                            <span>Course: {getCourseTitle(booking.courseId)}</span>
                          ) : booking.serviceId ? (
                            <span>Service: {getServiceTitle(booking.serviceId)}</span>
                          ) : (
      ...
      <Dialog open={openDialog} onOpenChange={setOpenDialog}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>Meeting Booking Details</DialogTitle>
            <DialogDescription>
              {selectedBooking && (
                <span>
                  ...
                  <CardContent className="space-y-3">
                    ...
                    <div>
                      ...
                      {selectedBooking.courseId ? (
                        <p>Course: {getCourseTitle(selectedBooking.courseId)}</p>
                      ) : selectedBooking.serviceId ? (
                        <p>Service: {getServiceTitle(selectedBooking.serviceId)}</p>
                      ) : (
                        <p>General inquiry</p>
                      )}
                    </div>
                ...
                <div className="space-x-2">
                  ...
                  <Button
                    onClick={() => {
                      ...
                      const event = {
                        text: `Meeting with ${selectedBooking.name}`,
                        dates: `${dateTime}/${endDateTime}`,
                        details: `Meeting regarding ${selectedBooking.courseId ? 'Course: ' + getCourseTitle(selectedBooking.courseId) : 
                                 selectedBooking.serviceId ? 'Service: ' + getServiceTitle(selectedBooking.serviceId) : 'General inquiry'}\n\n` +
                                 `${selectedBooking.message || ''}`
                      };
